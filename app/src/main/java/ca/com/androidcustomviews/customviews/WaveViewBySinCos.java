...
public class WaveViewBySinCos extends View {
    public static final String TAG = "WaveViewBySinCos";
...
//    wave:waveAmplitude="10dp" //  振幅
//    wave:waveFillType="bottom"//填充模式
//    wave:waveSpeed="3"//填充速度
//    wave:waveStart="true"//是否开始动画
//    wave:waveStartPeriod="1"//  开始位置相差多少个周期
//    wave:waveType="sin"  //波浪类型，是正弦还是余弦
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        ω = 2 * Math.PI / getWidth();//角速度  ω=Φ/t=2π/T=2πf说明：ω ，圆频率，也叫角速度．Φ，角度，t，时间π，圆周率T
        Log.e(TAG, "onSizeChanged:ω =  "+ω);
    }
    ...
    private void fillBottom(Canvas canvas) {
       φ -= waveSpeed / 100;
        ...
        for (float x = 0; x <= getWidth(); x += 20) {
//            y = (float) (A * Math.sin(ω * x + φ + Math.PI * startPeriod) + K);//0 ->30->0 这样轮询
//          Log.e(TAG, "fillBottom: =========>>>>  y = "+   y);
//            path.lineTo(x, y);
